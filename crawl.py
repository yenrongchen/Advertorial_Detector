...
OUTPUT_FILE = "dcard_travel_raw.json"  # 文章輸出檔案名稱
        ...
        try:
            while len(all_posts) < TARGET_AMOUNT:
                ...
                for widget in result.get("widgets", []):
                    # 直接是文章
                    if "post" in widget:
                        posts_this_page.append(widget["post"])
                    # 文章列表
                    elif "forumList" in widget:
                        for item in widget["forumList"].get("items", []):
                            if "post" in item:
                                posts_this_page.append(item["post"])

                print(f"  📥 這頁有 {len(posts_this_page)} 篇文章")
                ...
                for post in posts_this_page:
                    ...
                    print(f"  ✓ [{len(all_posts)}] {post.get('title', '')}")
